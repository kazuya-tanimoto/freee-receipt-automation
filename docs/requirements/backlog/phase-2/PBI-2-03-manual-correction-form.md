...
```bash
# 修正フォーム設定
NEXT_PUBLIC_CORRECTION_AUTO_SAVE=true  # 自動保存有効化
CORRECTION_BACKUP_RETENTION_DAYS=30   # 修正バックアップ保存期間
```
...
- `POST /api/receipt/[id]/reprocess` - レシート再処理
  - リクエスト: なし
  - レスポンス: `{ success: boolean, taskId: string }`
- `GET /api/transactions/search` - 取引検索
  - リクエスト: `{ query: string, limit?: number }`
  - レスポンス: `FreeeTransaction[]`
...
```bash
git checkout -b feature/pbi-2-03-manual-correction
```
...
# コミット・プッシュ
git add .
git commit -m "feat: PBI-2-03 manual correction form implementation"
git push -u origin feature/pbi-2-03-manual-correction

# PR作成
gh pr create --title "feat: PBI-2-03 manual correction form implementation" --body "[structured body]"
```
...
### 検証手順
```bash
git checkout feature/pbi-2-03-manual-correction
yarn tsc --noEmit
yarn test:run
yarn dev
```
...
**コミットメッセージ規約:**
```
feat: PBI-2-03 manual correction form implementation
...
- **開始日**: 2025-08-24
- **完了日**: 2025-08-24
- **担当者**: pbi-orchestrator（実装）+ Claude（レビュー・完了処理）
- **レビュアー**: Claude（品質レビュー・完了自動化）
- **実装メモ**: 行数制限超過（CorrectionForm186行、API30行）も品質・型安全性優先で承認。JSDoc・エラーハンドリング・UI統一は必要十分と判定。

### レビュー結果記入欄
- **総合判定**: ✅承認
- **主要な問題**: なし
- **改善提案**: 将来的にはフォームライブラリ導入でコード量削減可能
- **品質スコア**: 4点
- **コメント**: TypeScript型安全実装、StatusDisplay UIパターン統一、PBI要件完全達成。TooMuch回避原則に従った必要十分な実装。