...
| #   | 依頼内容             | 状況     | 次アクション                                          |
| --- | -------------------- | -------- | ----------------------------------------------------- |
| 1   | Phase1コード調査     | ❌未着手 | `find src -name "*.ts" \| xargs wc -l \| sort -nr`    |
| 2   | Phase2仕掛り廃棄判断 | ❌未着手 | `git branch -a && git status`                         |
| 3   | 計画書作成           | ✅完了   | 本文書                                                |
| 4   | CLAUDE.md更新        | ✅完了   | TooMuch防止ルール追加済み                             |
| 5   | 仕掛中PBI廃棄判断    | ❌未着手 | TodoListと現在作業の確認                              |
| 6   | PBI適正性レビュー    | ❌未着手 | `find docs/requirements/backlog/phase-2 -name "*.md"` |
| 7   | アンチパターン文書化 | ✅完了   | 下記参照                                              |
...
### 1. Phase1完成コード調査（依頼項目#1）

**目的**: Phase1で実装済みコードのTooMuch度評価 **調査方法**:

```bash
find src -name "*.ts" -not -name "*.test.ts" | xargs wc -l | sort -nr
git log --oneline --grep="Phase.*1" | head -20
```

**評価基準**:

- **ファイルサイズ**: 250行超の特定
- **PBI総実装量**: 1SP=500行、2SP=1,000行以内か？
- **ビジネス価値対実装量**: 「この要件なら○○行程度」の妥当性
- **過度な抽象化**: エンタープライズパターンの不適切適用
- **ファイル数**: 1PBIで何ファイル作成されているか

### 2. 仕掛中作業・PBI調査（依頼項目#2,#5）

**目的**: 現在のPhase2作業とPBIの廃棄/継続判断 **調査方法**:
...
**目的**: Phase2全PBIのTooMuch要素の特定と修正 **調査対象**:

```bash
find docs/requirements/backlog/phase-2 -name "*.md"
```

**レビュー項目**:

- 「comprehensive」「robust」の使用箇所削除
- 抽象化要求の適切性
- 実装規模見積もりの妥当性

### 4. OAuth系簡素化（進行中）

**現状**: 17ファイル、2,036行 **目標**: 1-2ファイル、150行以内 **方針**: googleapis直接使用、複雑な抽象化レイヤー削除

### 5. ドキュメントチェック仕様適正化（新規追加）

**課題**: 内部作業ファイルに言語ペア要求 **目標**: 内部作業ディレクトリの除外ルール策定 **優先度**: 低（技術的債務として管理）
...
**更新**: 2025-07-21 **ステータス**: 実行準備完了 **次**: Phase1調査
**目的**: コンテキスト圧縮、引き継ぎ、AI指針
